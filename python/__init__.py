...
def CLIP_VL_image_caption( image_filepath : str ) -> str:
	...
	conversation : list[dict] = [
		{ "role": "User", "content": "You are a captioning bot which receives images from the user that you must precisely caption based on the given prompt. This means you are in instruct-answer mode and not chat-mode. Follow the prompt carefully." },
...
# 	del inputs
# 	output : str = processor.decode(output[0], skip_special_tokens=True)
# 	return output
...
# def CLIP_VL_image_caption(image_filepath: str) -> str:
# 	with open('python/prompt.txt', 'r') as file:
# 		prompt : str = file.read().replace("<image_placeholder>\n", "")
# 	return caption_raw_image( Image.open(image_filepath), prompt )
